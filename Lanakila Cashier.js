...
app.AddLayout(lay);
 apb = MUI.CreateAppBar("Lanakila Cashier",  "");
 ...
 img = app.CreateImage("Img/Lanakila Cashier.png",0.5);
...
tr=2;
    lay = MUI.CreateLayout("Linear", "FillXY,VCenter")
        ...
        var headers = ["ID", "Name Pemilik", "Nama Tenan","Password Tenan"]
        var values = [
            "1:Jajat:tesUser:123",
            "2:Sansa Stark:tes:123",
   ...
   var fBt = MUI.CreateButtonFlat('EDIT')
        fBt.SetEnabled(false)
        ftr.AddChild(fBt)
        fBt.SetOnTouch(function(){
            table.GetSelectedRows(function(x){
                table.RemoveRows(x)
            })
        })
        ...
        table.SetOnSelectionStatus(function(c) {
            if(c) fBt.SetEnabled(true)
            else fBt.SetEnabled(false)
        })
  var fBn = MUI.CreateButtonFlat('TAMBAH')
       // fBtn.SetEnabled(false)
        ftr.AddChild(fBn)
        fBn.SetOnTouch(inputTenan)
    app.AddLayout(lay)
...
app.AddLayout(lay);
 ...
 pageLay2.AddChild(yx);
  lay.AddChild(pageLay2);
 lay.AddChild(apb);
    tef = MUI.CreateTEFilledIconLeft(0.8, "Left", "book", "KODE STRUK", true)
        pageLay2.AddChild(tef)
 
    
         btn1 = MUI.CreateButtonRaisedO("Get Data",0.3);
...
function tt()
{
 ...
 lay = MUI.CreateLayout("Absolute", "FillXY,Vcenter");
app.AddLayout(lay);
 apb = MUI.CreateAppBar("Kasir", "menu", "");
 apb.SetOnMenuTouch(OnMenuTouch1);
 apb.SetOnControlTouch(OnControlTouch);
 apb.GetHeight();
 pageLay = MUI.CreateLayout("Linear", "horizontal");
 ...
 yx = MUI.CreateTextH2("", 1,0.1);
 pageLay2.AddChild(yx);
 btn1 = MUI.CreateButtonRaisedO("[fa-pencil] PEMBAYARAN",0.3);
btn1.SetOnTouch(baya);
 pageLay.AddChild(btn1);
...
//btn6.SetVisibility("hide");
 pageLay5.AddChild(btn6);

 btn7 = MUI.CreateButtonRaisedO("[fa-pencil]TENAN",0.3);
btn7.SetOnTouch(listtenan);
 pageLay5.AddChild(btn7);
...
function Start() {
 ...
 apb = MUI.CreateAppBar("Kasir", "arrow_back", "search");
